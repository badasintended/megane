...
public class BarRenderer implements ITooltipRenderer {
    ...
    private String getValString(CompoundTag data) {
        double stored = Math.max(data.getDouble(MeganeUtils.key("stored")), 0);
        double max = Math.max(data.getDouble(MeganeUtils.key("max")), 0);
        String unit = data.getString(MeganeUtils.key("unit"));
        boolean verbose = data.getBoolean(MeganeUtils.key("verbose"));
    ...
    public Dimension getSize(CompoundTag data, ICommonAccessor accessor) {
        ...
        String prefix = data.getString(MeganeUtils.key("prefix"));
        if (data.getBoolean(MeganeUtils.key("translate"))) prefix = I18n.translate(prefix);
        prefix += ": ";
    ...
    public void draw(MatrixStack matrices, CompoundTag data, ICommonAccessor accessor, int x, int y) {
        ...
        int color = data.getInt(MeganeUtils.key("color"));
        ...
        MeganeUtils.drawTexture(matrices, TEXTURE, x + align, y, 100, 11, 0, 0, 1F, 0.5F, color);
        MeganeUtils.drawTexture(matrices, TEXTURE, x + align, y, (int) (ratio * 100), 11, 0, 0.5F, ratio, 1F, color);
