...
@Deprecated
public class ToolComponent implements IComponentProvider {

    public static final ToolComponent INSTANCE = new ToolComponent();
    ...
    public void appendBody(List<Text> tooltip, IDataAccessor accessor, IPluginConfig config) {
        if (!CONFIG.get().effectiveTool.isEnabled()) return;
        ...
        if (blockSettingsAccessor.getHardness() == 0.0F) {
            ...
            for (Map.Entry<Tag<Item>, ItemStack> entry : TOOLS.entrySet()) {
                ...
                if (ToolManager.handleBreakingSpeed(state, tool, accessor.getPlayer()) >= ToolMaterials.WOOD.getMiningSpeedMultiplier()) {
