...
public class ChickenProvider implements IEntityComponentProvider, IDataProvider<ResourceChickenEntity> {
    ...
    public void appendBody(ITooltip tooltip, IEntityAccessor accessor, IPluginConfig config) {
        ...
        if (config.getBoolean(MeganeResourceChickens.CONFIG_SHOW_DROP) && timer != null && ResourceChickens.CONFIGURATION.allowInWorldDrops.get() && !chicken.isBaby() && timer.eggLayTime != 0) {
    ...
    public void appendData(IDataWriter data, IServerAccessor<ResourceChickenEntity> accessor, IPluginConfig config) {
        ...
        if (config.getBoolean(MeganeResourceChickens.CONFIG_SHOW_DROP) && ResourceChickens.CONFIGURATION.allowInWorldDrops.get()) {
